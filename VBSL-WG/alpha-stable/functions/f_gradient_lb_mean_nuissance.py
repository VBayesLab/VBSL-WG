...
def fun_gradient_lb(s, theta_samples, mu_q, l_q, c, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics):
    ...
    np.fill_diagonal(diag_var, np.diag(np.nan_to_num(np.sqrt(sample_variance_q), nan=0))) 
    mean_nuissance_p1 = np.linalg.inv(np.identity(num_latent) / sigma_latent + diag_var.T @ np.linalg.inv(sample_variance_q) @ diag_var)
