...
from f_mean_adjust_ss_wasserstein import summary_statistics, mean_adjustment_summary_statistics  
...
def mean_adjustment_unbiased_log_likelihood(gs_adjusted_theta, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq):
    mean_adj_ss = mean_adjustment_summary_statistics(gs_adjusted_theta, n_samples, n_datasets, num_coeffs, num_latent, mixture_obj_seq)
    ...
    mean_u_est_log_likelihood = -1/2 * np.log(np.linalg.det(sample_variance)) - 1/2 * part1
...
def fun_gradient_lb(s, theta_samples, mu_q, l_q, c, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq):
    ...
    ss_q = summary_statistics(theta_q, n_samples, n_datasets, mixture_obj_seq)
    ...
    np.fill_diagonal(diag_var, np.diag(sqrtm(sample_variance_q)))
    ...
    # Find gradient of LB
    llh = mean_adjustment_unbiased_log_likelihood(adjusted_theta_q, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq)
