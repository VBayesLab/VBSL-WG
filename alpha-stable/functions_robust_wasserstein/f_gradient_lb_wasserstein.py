...
from functions_robust_wasserstein.f_var_adjust_ss_wasserstein import variance_adjustment_summary_statistics 

def prior(theta, num_coeffs): 
    log_prior = multivariate_normal.logpdf(theta, cov= 10 * np.identity(num_coeffs))
    return log_prior

def variance_adjustment_unbiased_log_likelihood(gs_adjusted_theta, gamma_samples, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq):
    va_unbiased_log_likelihood = []
    for i in range(gamma_samples):
        var_adj_ss = variance_adjustment_summary_statistics(gs_adjusted_theta[i,:], n_samples, n_datasets, num_coeffs, num_latent, mixture_obj_seq)
...
def fun_gradient_lb(s, theta_samples, mu_q, l_q, c, gamma_samples, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq):
    ...
    # Find gradient of LB
    llh = variance_adjustment_unbiased_log_likelihood(adjusted_theta_q, gamma_samples, n_samples, n_datasets, num_coeffs, num_latent, actual_summary_statistics, mixture_obj_seq)
